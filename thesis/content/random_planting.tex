\section{Full colouring in the random planted model} \label{app: plantedcolouring}
...
\begin{algorithm}[H]
    As long as there is a vertex $x$ that has colour $c_1$, and a colour $c_2\neq c_1$ such that $x$ has less than $\frac{1}{6k}d$ neighbors with colour $c_2$, uncolour $x$.%\andor{why does \cite{MR3536556-David16} need another condition?}
...
\begin{proof}
    ...
    \begin{lemma}\label[lemma]{lemma: largecolouredneighborhoodprob}
    ...
    Equality~\ref{eq:alpha1} follows by \cref{lemma: largecolouredneighborhoodprob} and that by \cref{lemma: ssveonesided}
