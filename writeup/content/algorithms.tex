...
\begin{proof}[Proof of \cref{thm:kalg-partial}]
    ...
    Then for each $i \in [k']$ the number of surviving vertices is at least $\bar{p}_i = \max\Paren{0, |\chi^{-1}(a_i^*)| - \sum_{\substack{a \neq a_i^{*} \in S_i}} |\chi^{-1}(a)|}$.
    Then the algorithm returns a $(k', 1 - \frac{1}{n} \sum_{i \in [k']} \bar{p}_i + 2\delta + 2\e)$-colouring for $G$.
...
If $\delta \ll \gamma^2$, then any two rows $M^{(a)}$ and $M^{(b)}$ of $M$ satisfy $\Norm{M^{(a)} - M^{(b)}} \geq \Omega(\gamma)$.
\end{claim}
\begin{proof}
