...
Then, for $\delta, \lambda > 0$ small enough, there exists an algorithm that runs in time $n^{O(1)} \cdot O_{\delta,\lambda}(1)$ and outputs a $(2, O(\lambda+\delta/\lambda))$-coloring for $G$.
\end{theorem}
\begin{proof}
...
Then, we can recover in time $n^{O(1)} \cdot O_{\delta,\lambda}(1)$ a list of $O_{\delta,\lambda}(1)$ bipartitions $\hat{\chi}: [n] \to [2]$ such that, for at least one of them, it holds up to a permutation of the color classes that $\mathbb{E}_{\bm{x} \sim \operatorname{Unif}([n])} \1[\hat\chi(x) \neq \chi(x)] \leq O(\lambda+\delta/\lambda)$.
As in the proof of \cref{thm:kalg-partial}, for each candidate bipartition we apply the rounding algorithm in \cref{lemma: generalrounding} on each of its color classes to obtain $2$ independent sets, and return the set of $2$ independent sets that cover most vertices of $G$.
