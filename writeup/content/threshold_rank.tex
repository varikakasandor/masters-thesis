\section{Proof of top threshold rank lower bound}
\label{sec:rank-lb}

In this section, we provide a generalized proof of Lemma 6.1 of \cite{barak2011rounding} that extends to bounded-norm symmetric matrices.
...
\begin{lemma}[Generalized restatement of Lemma 6.1 in \cite{barak2011rounding}]
    ...
    % Given a graph $G$ with adjacency matrix $A \in \set{0, 1}^{n \times n}$, define the degree matrix $D \in {R}^{n \times n}$ as the diagonal matrix with $D_{i, i} = \deg_G(i)$ and the normalized adjacency matrix $A \in {R}^{n \times n}$ as $A = D^{-1/2} A D^{-1/2}$.
...
\begin{proof}
    ...
    We use $\Lambda_{\geq 1- C\e} = \sum_{i=1}^{k} u_i u_i^{\top}$ to denote the subspace spanned by the eigenvectors with value at least $1- C\e$. 
